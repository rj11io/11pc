...
- Bookmark buttons and the Bookmarked filter: buttons using aria-pressed. Their bookmark icons are decorative and marked aria-hidden.
...
Bookmark buttons confirm bookmarked, removed, and unavailable outcomes through a polite live region. They stay disabled until browser storage has been checked, so a reader cannot start an operation whose state is still unknown.

The empty state is not just a message. It includes a button that clears the search text, selected tags, and Bookmarked state, so a reader filtered into a corner gets out without finding and emptying each control.
